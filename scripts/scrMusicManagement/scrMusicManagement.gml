...
global.music_data = ds_map_create();

function music_load_folder(folder) {
	for (var f = file_find_first(folder + "*.ogg", 0); f != ""; f = file_find_next()) 
	{
	    var name = filename_change_ext(f, "");
		var filename = $"{folder}/{name}.ogg";
		if file_exists(filename)
		{
		    global.music_data[? name] = audio_create_stream(filename);
	...
	//var filename = $"{folder}/{music}.ogg";
	//if file_exists(filename) {
	//	global.music_data[? music] = audio_create_stream(filename);
	//	print($"{filename} loaded succesfully");
	//}
}

music_load_folder("Music/Menu");
music_load_folder("Music/Minigames");
music_load_folder("Music/Minigames/1vs3");
music_load_folder("Music/Minigames/2vs2");
music_load_folder("Music/Minigames/4vs");
music_load_folder("Music/Party");
music_load_folder("Music/Results");
...
function music_set_loop_points(music) {	
	...
	if (variable_struct_exists(global.music_loop_points, music)) {
		var loop_point = global.music_loop_points[$ music];
